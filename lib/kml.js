...
/** Google Earth-style pin: TC = red circle, Start/Finish/Stop = red flag */
...
function parsePlacemark(xml, index, styleIndex) {
  ...
  for (const coordinates of parseGeometryBlocks(xml, "Point")) {
    ...
    items.push({
      ...
      geometryType: "Point",
      coordinates: [{ lat: point.lat, lon: point.lon, iconKind, iconHref: style.iconHref || null }],
      iconKind,
...
function parseKml(kmlText, sourceFile = "upload.kml") {
  ...
  blocks.forEach((block, index) => {
    for (const section of parsePlacemark(block, index, styleIndex)) {
