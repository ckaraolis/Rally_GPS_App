...
/** SHA-256 proof drivers can check offline without receiving the PIN. */
function offlineStopProof(password, salt) {
  return crypto.createHash("sha256").update(`${salt}:${String(password)}`, "utf8").digest("hex");
}
...
function verifyOfflineStopProof(password, salt, proof) {
  if (!salt || !proof) return false;
  const next = Buffer.from(offlineStopProof(password, salt), "hex");
  const prev = Buffer.from(String(proof), "hex");
  if (next.length !== prev.length) return false;
  return crypto.timingSafeEqual(next, prev);
}
