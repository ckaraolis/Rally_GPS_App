...
function toCar(row) {
  ...
  return {
    ...
    reconnectRequested: row.reconnect_requested ? Number(row.reconnect_requested) : null,
  };
}
...
function toRow(car) {
  return {
    ...
    reconnect_requested: car.reconnectRequested || null,
...
function createSupabaseStore() {
  ...
  return {
    ...
    async saveCar(car) {
      ...
      if (error && /reconnect_requested/.test(error.message || "")) {
        delete row.reconnect_requested;
        const retry = await supabase.from("rally_cars").upsert(row, { onConflict: "id" });
        error = retry.error;
      }
      if (error) throw error;
      return car;
