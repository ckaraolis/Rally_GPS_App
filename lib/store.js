...
function rallySummary(rally) {
  ...
  const { snapshot, pinIcons, driverStopSalt, driverStopHash, ...rest } = rally;
...
function createSupabaseStore() {
  ...
  return {
    ...
    async saveRally(rally) {
      ...
      if (
        error &&
        /driver_stop_(salt|hash)/i.test(error.message || "") &&
        (row.driver_stop_salt !== undefined || row.driver_stop_hash !== undefined)
      ) {
