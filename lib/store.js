...
function toCar(row) {
  ...
  const lat = rawLast != null ? Number(rawLast.lat) : NaN;
  const lon = rawLast != null ? Number(rawLast.lon) : NaN;
  const last =
    rawLast && Number.isFinite(lat) && Number.isFinite(lon)
      ? {
          ...rawLast,
          lat,
          lon,
          heading: Number.isFinite(Number(rawLast.heading)) ? Number(rawLast.heading) : null,
          speed: Number.isFinite(Number(rawLast.speed)) ? Number(rawLast.speed) : null,
          accuracy: Number.isFinite(Number(rawLast.accuracy)) ? Number(rawLast.accuracy) : null,
          ts: Number.isFinite(Number(rawLast.ts)) ? Number(rawLast.ts) : null,
        }
      : null;
  const trail = Array.isArray(row.trail) ? row.trail : [];
  const trailCount = Number.isFinite(Number(row.trail_count))
    ? Number(row.trail_count)
    : trail.length;
  return {
    id: row.id,
    token: row.token,
    ...
    pinIcons:
      row.pin_icons && typeof row.pin_icons === "object" && !Array.isArray(row.pin_icons)
        ...
        : Object.prototype.hasOwnProperty.call(row, "pin_icons")
          ? {}
          : undefined,
    driverStopSalt: Object.prototype.hasOwnProperty.call(row, "driver_stop_salt")
      ? row.driver_stop_salt || null
      : undefined,
    driverStopHash: Object.prototype.hasOwnProperty.call(row, "driver_stop_hash")
      ? row.driver_stop_hash || null
      : undefined,
    driverStopOffline: Object.prototype.hasOwnProperty.call(row, "driver_stop_offline")
      ? row.driver_stop_offline || null
      : undefined,
...
function toRallyRow(rally) {
  ...
  if (rally.driverStopSalt !== undefined) row.driver_stop_salt = rally.driverStopSalt || null;
  if (rally.driverStopHash !== undefined) row.driver_stop_hash = rally.driverStopHash || null;
  if (rally.driverStopOffline !== undefined) row.driver_stop_offline = rally.driverStopOffline || null;
  return row;
}

function rallySummary(rally) {
  if (!rally) return null;
  const { snapshot, pinIcons, driverStopSalt, driverStopHash, driverStopOffline, ...rest } = rally;
  ...
  return {
    ...rest,
    pinIconSlots,
    driverStopLock: Boolean(driverStopHash),
  };
}
...
function createMemoryStore() {
  ...
  return {
    mode: "memory",
    async listCars(opts = {}) {
      const includeTrail = Boolean(opts.includeTrail);
      return [...cars.values()].map((car) => {
        const trail = Array.isArray(car.trail) ? car.trail : [];
        return {
          ...car,
          trail: includeTrail ? trail : [],
...
function createSupabaseStore() {
  ...
  const CAR_LIST_COLS =
    "id,token,car_number,driver_name,color,tracking,last,section,crew_status,flag_ack,reconnect_requested,updated_at,trail_count";
  ...
  return {
    ...
    async listCars(opts = {}) {
      ...
      // Keep HQ/map polls light — full trails made /api/cars too large to load.
      let result = await supabase.from("rally_cars").select(CAR_LIST_COLS);
      if (result.error && /trail_count/i.test(result.error.message || "")) {
        result = await supabase
          .from("rally_cars")
          .select(
            "id,token,car_number,driver_name,color,tracking,last,section,crew_status,flag_ack,reconnect_requested,updated_at"
          );
      }
      if (result.error) throw result.error;
      return (result.data || []).map((row) => toCar({ ...row, trail: [] }));
    },
    async getCar(id) {
    ...
    async saveCar(car) {
      const row = toRow(car);
      let { error } = await supabase.from("rally_cars").upsert(row, {
        onConflict: "id",
      });
      if (error && /trail_count/i.test(error.message || "")) {
        delete row.trail_count;
        ({ error } = await supabase.from("rally_cars").upsert(row, { onConflict: "id" }));
      }
      if (error) throw error;
      return car;
    },
    ...
    async listRallies() {
      const base = "id,name,start_date,end_date,status,car_count,created_at,updated_at";
      let result = await supabase
        .from("rally_events")
        .select(`${base},driver_stop_hash`)
        .order("created_at", { ascending: false });
      if (result.error && /driver_stop_hash/i.test(result.error.message || "")) {
        result = await supabase.from("rally_events").select(base).order("created_at", { ascending: false });
      }
      if (result.error) throw result.error;
      return (result.data || []).map(toRally);
    },
    async getRally(id) {
      const { data, error } = await supabase
        .from("rally_events")
    ...
    async getLiveRally() {
      ...
      let result = await supabase
        .from("rally_events")
        .select(`${base},driver_stop_salt,driver_stop_hash`)
        .eq("status", "live")
        .maybeSingle();
      if (result.error && /driver_stop_(salt|hash)/i.test(result.error.message || "")) {
        result = await supabase.from("rally_events").select(base).eq("status", "live").maybeSingle();
      }
      if (result.error) throw result.error;
      return toRally(result.data);
    },
    async saveRally(rally) {
      ...
      if (
        error &&
        /driver_stop_(salt|hash|offline)/i.test(error.message || "") &&
        (row.driver_stop_salt !== undefined ||
          row.driver_stop_hash !== undefined ||
          row.driver_stop_offline !== undefined)
      ) {
        // Columns missing: still save the rally; caller keeps salt/hash in memory.
        delete row.driver_stop_salt;
        delete row.driver_stop_hash;
        delete row.driver_stop_offline;
        const retry = await supabase.from("rally_events").upsert(row, { onConflict: "id" });
        if (retry.error) throw retry.error;
        rally.stopLockDbMissing = true;
        return rally;
      }
