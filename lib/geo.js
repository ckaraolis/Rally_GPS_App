...
/**
 * Estimate progress along a LineString (0..1) via nearest point on the path.
 * Returns null if the path is too short or the point is farther than maxDistM.
 */
function progressAlongPath(point, coords, maxDistM = 200) {
  ...
  for (let i = 0; i < coords.length - 1; i++) {
    ...
    const P = toLocalXY(point.lat, point.lon, refLat);
    const A = toLocalXY(a.lat, a.lon, refLat);
    const B = toLocalXY(b.lat, b.lon, refLat);
    const abx = B.x - A.x;
    const aby = B.y - A.y;
    ...
    const dist = haversineMeters(point, closest);
    if (dist < bestDist) {
      bestDist = dist;
  ...
  return {
    ...
    alongM: Math.round(bestAlong),
    totalM: Math.round(total),
    nearestDistM: Math.round(bestDist),
  };
}
...
function detectSection(point, sections, options = {}) {
  ...
  for (const section of sections) {
    ...
    if (section.type === "marker" || section.geometryType === "Point") continue;
  ...
  return {
    ...
    flagTargets: Array.isArray(best.flagTargets) ? best.flagTargets.map(String) : [],
